...
func TestRateAnnotations(t *testing.T) {
	testCases := map[string]struct {
		...
		"info annotation when rate() over series with delta temporality": {
			data: `
				series{label="a", __temporality__="delta"} 1 2 3
			`,
			expr:                     "rate(series[1m1s])",
			typeAndUnitLabelsEnabled: true,
			expectedInfoAnnotations: []string{
				`PromQL info: rate() should not be used on delta temporality metrics. The rate is already calculated: "series" (1:6)`,
			},
			expectedWarningAnnotations: []string{},
		},
		"info annotation when increase() over series with delta temporality": {
			data: `
				series{label="a", __temporality__="delta"} 1 2 3
			`,
			expr:                     "increase(series[1m1s])",
			typeAndUnitLabelsEnabled: true,
			expectedInfoAnnotations: []string{
				`PromQL info: increase() should not be used on delta temporality metrics. The rate is already calculated: "series" (1:10)`,
			},
			expectedWarningAnnotations: []string{},
		},
		"no info annotation when rate() over series with cumulative temporality": {
			data: `
				series{label="a", __temporality__="cumulative"} 1 2 3
			`,
			expr:                       "rate(series[1m1s])",
			typeAndUnitLabelsEnabled:   true,
			expectedWarningAnnotations: []string{},
		...
		"no info annotation when increase() over series with cumulative temporality": {
			data: `
				series{label="a", __temporality__="cumulative"} 1 2 3
			`,
			expr:                       "increase(series[1m1s])",
...
func TestSumOverTimeAnnotations(t *testing.T) {
	testCases := map[string]struct {
		data                       string
		expr                       string
		typeAndUnitLabelsEnabled   bool
		...
		"info annotation when sum_over_time() over series with cumulative temporality": {
			data: `
				series{label="a", __temporality__="cumulative"} 1 2 3
			`,
			expr:                     "sum_over_time(series[1m1s])",
			typeAndUnitLabelsEnabled: true,
			expectedInfoAnnotations: []string{
				`PromQL info: sum_over_time() should not be used on cumulative temporality metrics. Use rate() or increase() for rate calculations: "series" (1:15)`,
			},
			expectedWarningAnnotations: []string{},
		},
		"no info annotation when sum_over_time() over series with delta temporality": {
			data: `
				series{label="a", __temporality__="delta"} 1 2 3
			`,
			expr:                       "sum_over_time(series[1m1s])",
		...
		"no info annotation when sum_over_time() over series without temporality": {
			data: `
				series{label="a"} 1 2 3
			`,
			expr:                       "sum_over_time(series[1m1s])",
		...
		"no info annotation when sum_over_time() without type and unit labels enabled": {
			data: `
				series{label="a", __temporality__="cumulative"} 1 2 3
			`,
			expr:                       "sum_over_time(series[1m1s])",
